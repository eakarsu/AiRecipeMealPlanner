import React, { useState } from 'react';
import api from '../api/axios';
import { showToast } from '../components/ToastProvider';

const COMMON_ALLERGENS = ['Peanuts', 'Tree Nuts', 'Milk', 'Eggs', 'Wheat', 'Soy', 'Fish', 'Shellfish', 'Sesame', 'Mustard'];

function AllergenDetection() {
  const [recipeText, setRecipeText] = useState('');
  const [allergies, setAllergies] = useState([]);
  const [customAllergy, setCustomAllergy] = useState('');
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState(null);
  const [error, setError] = useState('');

  const toggleAllergy = (a) => {
    setAllergies(prev => prev.includes(a) ? prev.filter(x => x !== a) : [...prev, a]);
  };

  const addCustom = () => {
    const val = customAllergy.trim();
    if (val && !allergies.includes(val)) {
      setAllergies(prev => [...prev, val]);
      setCustomAllergy('');
    }
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!recipeText.trim()) {
      setError('Please paste a recipe to scan.');
      return;
    }
    if (!allergies.length) {
      setError('Please select at least one allergy to check against.');
      return;
    }
    setLoading(true); setError(''); setResult(null);
    try {
      const res = await api.post('/ai/allergen-detection', { recipe: recipeText, allergies });
      setResult(res.data?.detection || res.data || null);
      if (!res.data?.detection && !res.data) {
        showToast.info('AI returned an unstructured response.');
      }
    } catch (err) {
      const msg = err.response?.data?.error || 'Failed to detect allergens.';
      setError(msg);
      showToast.error(msg);
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="max-w-5xl mx-auto py-6 px-4 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Allergen Detection</h1>
        <p className="text-gray-600 text-sm mt-1">Scan a recipe for allergens, cross-contamination risks, and substitutions.</p>
      </div>

      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-2">Recipe text</label>
          <textarea
            value={recipeText}
            onChange={e => setRecipeText(e.target.value)}
            rows={8}
            placeholder="Paste recipe ingredients and steps here..."
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent"
          />
        </div>

        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-2">Allergies / sensitivities</label>
          <div className="flex flex-wrap gap-2 mb-3">
            {COMMON_ALLERGENS.map(a => (
              <button
                type="button"
                key={a}
                onClick={() => toggleAllergy(a)}
                className={`px-3 py-1.5 rounded-full text-sm border transition ${
                  allergies.includes(a)
                    ? 'bg-purple-600 border-purple-600 text-white'
                    : 'bg-white border-gray-300 text-gray-700 hover:border-purple-400'
                }`}
              >
                {a}
              </button>
            ))}
          </div>
          <div className="flex gap-2">
            <input
              type="text"
              value={customAllergy}
              onChange={e => setCustomAllergy(e.target.value)}
              placeholder="Add a custom allergen"
              className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent"
            />
            <button type="button" onClick={addCustom} className="px-4 py-2 rounded-lg bg-gray-100 text-gray-700 hover:bg-gray-200 text-sm font-medium">
              Add
            </button>
          </div>
          {allergies.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-3 text-xs text-gray-600">
              Selected: {allergies.map(a => <span key={a} className="bg-purple-100 text-purple-700 px-2 py-0.5 rounded">{a}</span>)}
            </div>
          )}
        </div>

        {error && <div className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{error}</div>}

        <div>
          <button type="submit" disabled={loading} className="px-5 py-2.5 rounded-lg bg-purple-600 text-white text-sm font-medium hover:bg-purple-700 disabled:opacity-50">
            {loading ? 'Scanning…' : 'Scan Recipe'}
          </button>
        </div>
      </form>

      {loading && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex items-center gap-3 text-gray-600">
          <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-purple-600" />
          Checking for allergens…
        </div>
      )}

      {result && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
          <h2 className="text-lg font-semibold text-gray-900">Detection Result</h2>
          {result.summary && <p className="text-sm text-gray-700">{result.summary}</p>}
          {typeof result.risk_level === 'string' && (
            <div className="text-sm">
              <span className="font-semibold text-gray-700">Risk Level: </span>
              <span className="text-gray-900">{result.risk_level}</span>
            </div>
          )}

          <Section title="Detected Allergens" items={result.detected_allergens} />
          <Section title="Cross-Contamination Risks" items={result.cross_contamination_risks} />
          {Array.isArray(result.substitutions) && result.substitutions.length > 0 && (
            <div>
              <h3 className="text-sm font-semibold text-gray-700 mb-2">Suggested Substitutions</h3>
              <ul className="text-sm text-gray-700 space-y-1 list-disc pl-5">
                {result.substitutions.map((s, i) => (
                  <li key={i}>{typeof s === 'string' ? s : (s.replace ? <><strong>{s.replace}</strong> &rarr; {s.with || s.alternative}</> : JSON.stringify(s))}</li>
                ))}
              </ul>
            </div>
          )}
          <Section title="Hidden Sources" items={result.hidden_sources} />
          <Section title="Recommendations" items={result.recommendations} />
        </div>
      )}
    </div>
  );
}

function Section({ title, items }) {
  if (!Array.isArray(items) || items.length === 0) return null;
  return (
    <div>
      <h3 className="text-sm font-semibold text-gray-700 mb-2">{title}</h3>
      <ul className="text-sm text-gray-700 space-y-1 list-disc pl-5">
        {items.map((it, i) => <li key={i}>{typeof it === 'string' ? it : JSON.stringify(it)}</li>)}
      </ul>
    </div>
  );
}

export default AllergenDetection;
